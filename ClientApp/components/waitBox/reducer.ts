...
class WaitReducer extends ReducerBase<IWaitBoxState, myActions> {
  createActionList() {
    return {
      ...
      executeWait: [
        ...
        (waitId: string, description: string, loadPromise: PromiseLike<boolean>) => ({
          Async: true,
          waitId,
          description,
        }),
  ...
  reducers() {
    ...
    waitListHandlers[this._myActions.executeWait.toString()] = (state, action) => {
      ...
      if (payload.isLoading || payload.error) {
        ...
        if (currentState[meta.waitId]) delete currentState[meta.waitId];
      }
    ...
    waitListHandlers[this._myActions.cancelWait.toString()] = (state, action) => {
      ...
      if (!currentState[key] || currentState[key].waitStatusAsync.isLoading) return state;
  ...
  doWaitAsync = (description: string, loader: () => PromiseLike<boolean>) =>
    this.doWait(description, loader()) as any;
  ...
  setProgress = (waitId: string, progress?: WaitProgress) =>
    this._myActions.setProgress(waitId, progress);
