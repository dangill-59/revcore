...
export function AuthPopup(
  url: string,
  tokenNames?: string[],
  options?: { title?: string; width?: number; height?: number; existingWindow?: Window },
) {
  options = options || {};
  if (!options.width) options.width = window.innerWidth - 10;
  if (!options.height) options.height = window.innerHeight - 10;
  ...
  if (!tokenNames || tokenNames.length == 0) tokenNames = ['code'];

  const externalWindow: any =
    (options && options.existingWindow) ||
    window.open(
      url,
  ...
  if (null == externalWindow) throw 'Failed to open poup';
...
export default class OauthPopup extends React.PureComponent<props> {
  ...
  componentWillUnmount() {
    if (this.auth) return this.auth.close();
  }
}
