...
function testV2ToSign(t, request, amzHeaders, addConfig, evapConfig) {
  return testV2Authorization(t, evapConfig, addConfig).then(function () {
    var qp = params(testRequests[t.context.testId][2].url),
      ...
      r = Object.assign({}, request, {
        x_amz_headers: h,
        contentType: addConfig ? addConfig.contentType : undefined,
      }),
      expected = encodeURIComponent(
        stringToSignV2(
          '/' +
            AWS_BUCKET +
            '/' +
            t.context.config.name +
            '?partNumber=1&uploadId=Hzr2sK034dOrV4gMsYK.MMrtWIS8JVBPKgeQ.LWd6H8V2PsLecsBqoA1cG1hjD3G4KRX_EBEwxWWDu8lNKezeA--',
          'PUT',
          r,
        ),
      );
        ...
        expected = encodeURIComponent(
          stringToSignV2(
            '/' +
              AWS_BUCKET +
              '/' +
              t.context.originalUploadObjectKey +
              '?uploadId=Hzr2sK034dOrV4gMsYK.MMrtWIS8JVBPKgeQ.LWd6H8V2PsLecsBqoA1cG1hjD3G4KRX_EBEwxWWDu8lNKezeA--',
            'GET',
            r,
          ),
        );
...
function v4Authorization(signingKey) {
  return (
    'AWS4-HMAC-SHA256 Credential=testkey/' +
    v4DateString() +
    '/us-east-1/s3/aws4_request, SignedHeaders=host;testid;x-amz-date, Signature=' +
    signingKey
  );
}
...
function testV4ToSign(t, addConfig) {
  return testV4Authorization(
    t,
    {
      cryptoHexEncodedHash256: function (d) {
        return d;
      },
    },
    addConfig,
...
test('should correctly create V2 string to sign for PUT with amzHeaders', (t) => {
  return testV2ToSign(
    t,
    {},
    { 'x-custom-header': 'peanuts' },
    { xAmzHeadersCommon: { testId: t.context.testId, 'x-custom-header': 'peanuts' } },
...
test('should correctly create V2 string to sign with part-number-marker', (t) => {
  return testV2ListParts(t, {}, {}, {}, 5, 0).then(function (result) {
...
test('should correctly create V2 string to sign for truncated list parts', (t) => {
  ...
  return testV2ListParts(t, {}, {}, config, 5, 0).then(function (result) {
    expect(result.result).to.equal(result.expected);
  });
});

test('should correctly create V4 string to sign for PUT', (t) => {
  return testV4ToSign(t).then(function (result) {
    expect(result.result).to.equal(
      'AWS4-HMAC-SHA256%0A' +
        result.datetime +
        '%0A' +
        result.datetime.slice(0, 8) +
        '%2Fus-east-1%2Fs3%2Faws4_request%0APUT%0A%2Fbucket%2F' +
        t.context.requestedAwsObjectKey +
        '%0ApartNumber%3D1' +
        ...
        '%0Atestid%3A' +
        encodeURIComponent(t.context.testId) +
        '%0Ax-amz-date%3A' +
        result.datetime +
        '%0A%0Acontent-md5%3Bhost%3Btestid%3Bx-amz-date%0AUNSIGNED-PAYLOAD',
    );
  });
});
test('should correctly create V4 string to sign for PUT with amzHeaders', (t) => {
  return testV4ToSign(t, { xAmzHeadersCommon: { 'x-custom-header': 'peanuts' } }).then(
    function (result) {
      expect(result.result).to.equal(
        'AWS4-HMAC-SHA256%0A' +
          result.datetime +
          ...
          '%0Ax-amz-date%3A' +
          result.datetime +
          '%0Ax-custom-header%3Apeanuts%0A%0Acontent-md5%3Bhost%3Btestid%3Bx-amz-date%3Bx-custom-header%0AUNSIGNED-PAYLOAD',
      );
    },
  );
});
test('should correctly create V4 string to sign with part-number-marker', (t) => {
  return testV4ListParts(t, {}, 5, 0).then(function (result) {
    expect(testRequests[t.context.testId][19].url).to.match(/part-number-marker=2/);
  });
});
  ...
  return testV4ListParts(t, {}, 5, 0).then(function (result) {
    expect(result.result).to.equal(
      'AWS4-HMAC-SHA256%0A' +
        ...
        '%0A' +
        result.datetime.slice(0, 8) +
        '%2Fus-east-1%2Fs3%2Faws4_request%0AGET%0A%2Fbucket%2F' +
        t.context.originalUploadObjectKey +
        ...
        '%0Ahost%3As3.amazonaws.com%0Atestid%3A' +
        encodeURIComponent(t.context.testId) +
        '%0Ax-amz-date%3A' +
        result.datetime +
        '%0A%0Ahost%3Btestid%3Bx-amz-date%0A',
    );
  });
});
...
test('should fetch V2 authorization from the signerUrl without errors', (t) => {
  return testV2Authorization(t).then(function () {
...
test('should fetch V2 authorization from the signerUrl without canonical_request parameter present', (t) => {
  return testV4Authorization(t).then(function () {
...
test('should fetch V2 authorization using the signResponseHandler and signerUrl without errors', (t) => {
  return testV2Authorization(t, { signResponseHandler: signResponseHandler }).then(function () {
    expect(t.context.errMessages.length).to.equal(0);
  });
});
...
test('should call signResponseHandler() with the correct number of parameters', (t) => {
  ...
  return testV2Authorization(t, { signResponseHandler: handler }).then(function () {
...
test('should fetch V2 authorization using the customAuthMethod without errors', (t) => {
  return testV2Authorization(t, { signerUrl: undefined, customAuthMethod: customAuthHandler }).then(
    function () {
      expect(t.context.errMessages.length).to.equal(0);
    },
  );
...
test('should fetch V2 authorization using the customAuthMethod with the correct number of parameters', (t) => {
  ...
  return testV2Authorization(t, { signerUrl: undefined, customAuthMethod: customAuth }).then(
    function () {
...
test('should fetch V4 authorization using the signResponseHandler and signerUrl without errors', (t) => {
  return testV4Authorization(t, { signResponseHandler: signResponseHandler }).then(function () {
    expect(t.context.errMessages.length).to.equal(0);
  });
});
...
test('should fetch V4 authorization from the signerUrl with canonical_request parameter present, if enabled', (t) => {
  return testV4Authorization(t, { sendCanonicalRequestToSignerUrl: true }).then(function () {
    const x = testRequests[t.context.testId][0];
...
test('should fetch V4 authorization using the customAuthMethod without errors', (t) => {
  return testV4Authorization(t, { signerUrl: undefined, customAuthMethod: customAuthHandler }).then(
    function () {
      expect(t.context.errMessages.length).to.equal(0);
    },
  );
...
test('should fetch V4 authorization using the customAuthMethod with the correct number of parameters', (t) => {
  const customAuth = sinon.spy(customAuthHandler);
  return testV4Authorization(t, { signerUrl: undefined, customAuthMethod: customAuth }).then(
    function () {
      const a = Array.prototype.slice.call(customAuth.firstCall.args);
      expect(a.length).to.equal(5);
    },
...
test('should fetch authorization using a custom Authorization Method (awsLambda)', (t) => {
  ...
  return testV2Authorization(t, {
    customAuthMethod: authorizationMethod,
    signerUrl: undefined,
  }).then(function () {
    expect(t.context.errMessages.length).to.equal(0);
  });
...
test('should fetch V4 authorization with header "x-amz-content-sha256" for COMPLETE', (t) => {
  return testV4Authorization(t).then(function () {
    expect(headersForMethod(t, 'POST', /.*\?uploadId.*$/)['x-amz-content-sha256']).to.equal(
...
test('should abort on 404 in V2 Signature PUT should return errors and call the correct signer url', (t) => {
  ...
  return testV2Authorization(t).then(
    function () {
...
test('should abort on 404 in V2 Signature PUT should return errors and return error messages', (t) => {
  ...
  return testV2Authorization(t).then(
    ...
    function (reason) {
      expect(t.context.errMessages.join(',')).to.match(
        /404 error on part PUT. The part and the file will abort/i,
      );
    },
  );
});
