...
function testCancelCallbacks(t) {
  ...
  return testCommon(t, config, evapConfig).then(function () {
...
test('#create should return supported instance', (t) => {
  return Evaporate.create(baseConfig).then(
    function (evaporate) {
...
test('#create evaporate should respect localTimeOffset', (t) => {
  ...
  return Evaporate.create(config).then(
    function (evaporate) {
...
test('#create evaporate should respect returned server time from timeUrl when local is behind server', (t) => {
  var config = Object.assign({}, baseConfig, {
    timeUrl: 'http://example.com/time?testId=' + t.context.testId,
  });
...
test('new Evaporate() should instantiate and not call timeUrl', (t) => {
  ...
  return evaporateAdd(t, evaporate, config).then(function () {
...
test('should signerUrl is required unless signResponseHandler is present', (t) => {
  return Evaporate.create({ signerUrl: null, signResponseHandler: null }).then(
    function () {
      t.fail('Evaporate instantiated but should not have.');
    },
    function (reason) {
...
test('should require an AWS bucket with a signerUrl', (t) => {
  return Evaporate.create({ signerUrl: 'https://sign.com/sign' }).then(
    function () {
      t.fail('Evaporate instantiated but should not have.');
    },
    function (reason) {
...
test('should require an AWS bucket without a signerUrl but with a signResponseHandler', (t) => {
  return Evaporate.create({ signResponseHandler: function () {} }).then(
    function () {
      t.fail('Evaporate instantiated but should not have.');
    },
    function (reason) {
...
test.serial(
  'should require browser FileReader#readAsArrayBuffer support if computeContentMd5 enabled',
  (t) => {
...
test('should fail to add() when no file is present', (t) => {
  return Evaporate.create(baseConfig).then(function (evaporate) {
    evaporate.add({ name: 'test' }).then(
      function () {
...
test('should respect maxFileSize', (t) => {
  return Evaporate.create(Object.assign({}, baseConfig, { maxFileSize: 10 })).then(
    function (evaporate) {
      evaporate
        .add({
          file: new File({
...
test('should add() new upload with correct config', (t) => {
  return testCommon(t).then(function (fileKey) {
    let id = fileKey;
    expect(id).to.equal(t.context.requestedAwsObjectKey);
  });
});
test('should add() new upload with correct completed XML', (t) => {
  return testCommon(t).then(function () {
    expect(testRequests[t.context.testId][5].requestBody).to.equal(
...
test('should return fileKeys correctly for common cases started', (t) => {
  ...
  let start_id;
  return testCommon(t, config).then(function () {
...
test('should return the object key in the complete callback', (t) => {
  ...
  let config = Object.assign({}, baseAddConfig, {
    complete: sinon.spy(function (xhr, name) {
      complete_id = name;
    }),
  });
...
test('should call a callback on successful add()', (t) => {
  return testCommon(t).then(function () {
    expect(
      t.context.config.started.withArgs(baseConfig.bucket + '/' + t.context.requestedAwsObjectKey)
        .calledOnce,
    ).to.be.true;
  });
});
...
test('should call a callback on successful initiate()', (t) => {
  ...
  return testCommon(t, config).then(function () {
    expect(
      initated_spy.withArgs(
        'Hzr2sK034dOrV4gMsYK.MMrtWIS8JVBPKgeQ.LWd6H8V2PsLecsBqoA1cG1hjD3G4KRX_EBEwxWWDu8lNKezeA--',
      ).calledOnce,
    ).to.be.true;
  });
});
...
test('should call a progress with stats callback on successful add()', (t) => {
  return testCommon(t, { progress: sinon.spy() }).then(function () {
...
test('should cancel() two uploads with correct id, first result OK', (t) => {
  ...
  return Promise.all([promise0, promise1]).catch(function (reason) {
    t.fail('Promises failed.');
  });
});

test('should call a callbacks on cancel(): canceled', (t) => {
  return testCancelCallbacks(t).then(function () {
...
test('should add() new upload with correct config with custom bucket on add', (t) => {
  ...
  return Promise.all([a1, a2]).then(function () {
