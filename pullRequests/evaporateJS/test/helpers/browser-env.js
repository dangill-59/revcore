...
FileReaderMock.prototype.readAsArrayBuffer = function () {
  this.onloadend();
};
...
global.serverCommonCase = function (partRequestHandler) {
  ...
  server.respondWith('POST', /^.*\?uploads.*$/, (xhr) => {
    ...
    xhr.respond(
      retryStatus(xhr, 'init'),
      CONTENT_TYPE_XML,
      initResponse(AWS_BUCKET, AWS_UPLOAD_KEY),
    );
  });
  ...
  server.respondWith('POST', /.*\?uploadId.*$/, (xhr) => {
    storeTestRequest(xhr);
    xhr.respond(
      retryStatus(xhr, 'complete'),
      CONTENT_TYPE_XML,
      completeResponse(AWS_BUCKET, AWS_UPLOAD_KEY),
    );
  });
  ...
  server.respondWith('GET', /.*\?uploadId.*$/, (xhr) => {
    ...
    xhr.respond(
      status,
      CONTENT_TYPE_XML,
      getPartsResponse(AWS_BUCKET, AWS_UPLOAD_KEY, maxParts, marker++),
    );
...
global.beforeEachSetup = function (t, file) {
  ...
  t.context.cryptoMd5 = sinon.spy(function () {
    return 'md5Checksum';
  });
  t.context.cryptoHexEncodedHash256 = sinon.spy(function () {
    return 'SHA256Value';
  });
...
global.newEvaporate = function (t, evapConfig) {
  ...
  t.context.evaporate = new Evaporate(
    Object.assign({}, baseConfig, { cryptoMd5Method: t.context.cryptoMd5 }, evapConfig, {
...
global.evaporateAdd = function (t, evaporate, addConfig, configOverrides) {
  ...
  if (addConfig.xAmzHeadersAtUpload || addConfig.xAmzHeadersAtComplete) {
    addConfig.xAmzHeadersAtUpload = Object.assign(
      { testId: t.context.testId },
      addConfig.xAmzHeadersAtUpload,
    );

    addConfig.xAmzHeadersAtComplete = Object.assign(
      { testId: t.context.testId },
      addConfig.xAmzHeadersAtComplete,
    );

    if (addConfig.xAmzHeadersCommon) {
      addConfig.xAmzHeadersCommon = Object.assign(
        {},
        { testId: t.context.testId },
...
global.testBase = function (t, addConfig, evapConfig) {
  ...
  t.context.evapConfig = Object.assign(
    {},
    baseConfig,
    { cryptoMd5Method: t.context.cryptoMd5 },
    evapConfig,
    {
      signHeaders: Object.assign(
        {
  ...
  return Evaporate.create(t.context.evapConfig).then(function (evaporate) {
