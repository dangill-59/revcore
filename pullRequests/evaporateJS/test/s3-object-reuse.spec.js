...
function testS3Reuse(t, addConfig2, headEtag, evapConfig2, mockConfig) {
  ...
  return testBase(t, {}, Object.assign({}, evapConfig, mockConfig)).then(function () {
...
test('should re-use S3 object and callback complete', (t) => {
  return testS3Reuse(t, {}, '"b2969107bdcfc6aa30892ee0867ebe79-1"').then(function () {
    expect(t.context.config.complete.calledOnce).to.be.true;
  });
});
test('should re-use S3 object with S3 requests correctly ordered', (t) => {
  return testS3Reuse(t, {}, '"b2969107bdcfc6aa30892ee0867ebe79-1"').then(function () {
    expect(requestOrder(t)).to.equal('initiate,PUT:partNumber=1,PUT:partNumber=2,complete,HEAD');
  });
});
test.serial(
  'should not re-use S3 object with S3 requests correctly ordered, mocking localStorage',
  (t) => {
    return testS3Reuse(
      t,
      {},
      '"b2969107bdcfc6aa30892ee0867ebe79-1"',
      {},
...
test.serial(
  'should not re-use S3 object with S3 requests correctly ordered, no localStorage, not mocking',
  (t) => {
...
test.serial(
  'should not re-use S3 object calling cryptomd5 correctly, mocking localStorage',
  (t) => {
    return testS3Reuse(
      t,
      {},
...
test.serial(
  'should not re-use S3 object returning the S3 file upload ID, mocking localStorage',
  (t) => {
    return testS3Reuse(
      t,
      {},
...
test.serial(
  'should not re-use S3 object returning the S3 file upload ID, no localStorage, not mocking',
  (t) => {
    global['localStorage'] = undefined;
    return testS3Reuse(t, {}, '"b2969107bdcfc6aa30892ee0867ebe79-1"').then(function () {
      global.localStorage = storage;
...
test('should re-use S3 object and callback with the new object name', (t) => {
  return testS3Reuse(t, { nameChanged: sinon.spy() }, '"b2969107bdcfc6aa30892ee0867ebe79-1"').then(
    function () {
...
test("should not re-use S3 object if the first part's md5 digest do not match", (t) => {
  var cryptoMd5 = sinon.spy(function () {
    return 'md5Mismatch';
  });

  return testS3Reuse(t, {}, '"b2969107bdcfc6aa30892ee0867ebe79-1"', {
    cryptoMd5Method: cryptoMd5,
  }).then(function () {
    expect(requestOrder(t)).to.equal(
      'initiate,PUT:partNumber=1,PUT:partNumber=2,complete,initiate,PUT:partNumber=1,PUT:partNumber=2,complete',
    );
...
test('should not re-use S3 object because the Etag does not match and callback complete', (t) => {
  return testS3Reuse(t, {}, '"b2969107bdcfc6aa30892eeunmatched-1"').then(function () {
    expect(t.context.config.complete.calledOnce).to.be.true;
  });
});
...
test('should set xAmzHeadersCommon when re-using S3 object', (t) => {
  ...
  return testS3Reuse(t, config).then(function () {
...
test('should not retry HEAD when trying to reuse S3 object and status is 404 with complete callback', (t) => {
  ...
  return testS3Reuse(t).then(function () {
    expect(t.context.config.complete.calledOnce).to.be.true;
  });
});
...
test('should not callback with a new object name if status is non-404 error with a changed upload ID', (t) => {
  ...
  return testS3Reuse(t, { nameChanged: sinon.spy() }).then(function () {
    expect(t.context.config.nameChanged.called).to.be.false;
  });
});
