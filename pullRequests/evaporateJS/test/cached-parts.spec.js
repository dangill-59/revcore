...
function testCachedParts(t, addConfig, maxGetParts, partNumberMarker, evapCfg) {
  ...
  return testBase(t, addConfig, Object.assign({}, evapConfig, evapCfg)).then(function () {
    partNumberMarker = 0;
...
test('should check for parts when re-uploading a cached file and not call cryptoMd5', (t) => {
  return testCachedParts(t, {}, 1, 0).then(function () {
    expect(t.context.cryptoMd5.called).to.be.false;
  });
});
test.serial(
  'should check for parts when re-uploading a cached file and not call cryptoMd5, mocking localStorage',
  (t) => {
    return testCachedParts(t, {}, 1, 0, { mockLocalStorage: true }).then(function () {
      expect(t.context.cryptoMd5.called).to.be.false;
    });
  },
);
test('should check for parts when re-uploading a cached file with S3 requests in the correct order', (t) => {
  return testCachedParts(t, {}, 1, 0).then(function () {
    expect(requestOrder(t)).to.equal(
      'initiate,PUT:partNumber=1,PUT:partNumber=2,complete,check for parts,PUT:partNumber=2,complete',
    );
  });
});
test.serial(
  'should check for parts when re-uploading a cached file with S3 requests in the correct order, mocking localStorage',
  (t) => {
    return testCachedParts(t, {}, 1, 0, { mockLocalStorage: true }).then(function () {
      expect(requestOrder(t)).to.equal(
        'initiate,PUT:partNumber=1,PUT:partNumber=2,complete,check for parts,PUT:partNumber=2,complete',
...
test.serial(
  'should not check for parts when re-uploading a cached file with S3 requests in the correct order, no localStorage, not mocking',
  (t) => {
...
test('should check for parts when re-uploading a cached file and callback with the original object name', (t) => {
  return testCachedParts(
    ...
    expect(t.context.config.nameChanged.withArgs(t.context.originalUploadObjectKey).calledOnce).to
      .be.true;
  });
});
...
test('should check for parts when re-uploading a cached file, when getParts returns none and callback started', (t) => {
  return testCachedParts(t, {}, 0, 0).then(function () {
    expect(t.context.config.started.calledOnce).to.be.true;
  });
});
...
test('should check for parts when re-uploading a cached file, when getParts is truncated and callback started', (t) => {
  ...
  return testCachedParts(t, addConfig, 5, 0).then(function () {
    expect(t.context.config.started.calledOnce).to.be.true;
  });
});
...
test('should not retry check for remaining uploaded parts if status is 404', (t) => {
  t.context.getPartsStatus = 404;
  return testCachedParts(t, {}).then(function () {
    expect(requestOrder(t)).to.equal(
      'initiate,PUT:partNumber=1,PUT:partNumber=2,complete,' +
        'check for parts,' +
        'initiate,PUT:partNumber=1,PUT:partNumber=2,complete',
