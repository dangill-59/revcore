  ...
  function partRequestHandler(xhr, context) {
    if (xhr.url.indexOf('partNumber=1') > -1) {
      if (context.pauseHandler) {
        ...
        context.pausePromise = context.pause().then(function () {
...
test.beforeEach((t) => {
  ...
  t.context.pause = function () {
    return t.context.evaporate.pause(t.context.pauseFileId || t.context.uploadId, {
      force: t.context.force,
    });
  };
...
test('should Resume an upload and not call cryptoMd5', (t) => {
  return testPauseResume(t).then(function () {
...
test('should Resume an upload and callback started twice with the file key', (t) => {
  return testPauseResume(t).then(function () {
    expect(
      t.context.config.started.withArgs(AWS_BUCKET + '/' + t.context.requestedAwsObjectKey)
        .calledTwice,
    ).to.be.true;
  });
});
...
test('should fail to pause() when file not added', (t) => {
  ...
  return testPauseResume(t).then(function () {
    return t.context.pausePromise.then(
      function () {
...
test('should fail to pause() when file already paused', (t) => {
  ...
  return Promise.race([testPauseResume(t), pausePromise]).then(
    function () {
      t.fail('Expected test to fail.');
    },
    function (reason) {
...
test('should fail to resume() when file not added', (t) => {
  ...
  return testPauseResume(t).then(function () {
    return t.context.resumePromise.then(
      function () {
        t.fail('Expected test to fail.');
      },
      function (reason) {
