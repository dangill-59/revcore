...
import { createInjectStore, injectReducer } from 'redux-injector/src/ReduxInjector';
...
export function createInjectableStore(reducers, initialState, ...args) {
  ...
  const nonExisting = _.filter(
    _.keys(initialState || {}),
    (key) => !_.includes(existingReducerKeys, key),
  );

  let dummyReducers = {};
  _.each(nonExisting, (key) => {
    dummyReducers[key] = handleAction(
      createAction('DUMMYACTION_' + key),
      (state, action) => {
        return null;
      },
      null,
  ...
  return createInjectStore(_.assign(dummyReducers, reducers), initialState, ...args);
}
...
abstract class ReducerBase<TState, TAction = any> {
  ...
  protected getMyNameinStore() {
    return this._NameinStore;
  }
  ...
  private initialize(name: string) {
    ...
    injectReducer(name, combineReducers<TState>(this.reducers() as any), true);
    console.debug('injectreducer: injectReducer called');

    _instanceMap[name] = this;
  }

  public static getInstance<TState, T extends ReducerBase<TState>>(
    c: { new (): T },
    name: string,
  ): T {
